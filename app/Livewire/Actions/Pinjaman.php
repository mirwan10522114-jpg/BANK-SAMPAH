...
<<<<<<< HEAD
        ...
        // disbursements never collide on the unique nomor_pinjaman column.
...
<<<<<<< HEAD
            ...
            // ledger, sehingga kas "hilang" dari pembukuan.
            if ((float) $pinjaman->biaya_admin > 0) {
                KoperasiKasTransaksi::create([
                    'nomor_referensi' => $nomorPinjaman,
                    'sumber' => 'pinjaman',
                    'tipe' => 'masuk',
                    'jumlah' => $pinjaman->biaya_admin,
                    'keterangan' => 'Biaya Admin Pinjaman a.n '.$namaAnggota,
                    'tanggal_transaksi' => $this->tanggal_pengajuan,
                    'user_id' => Auth::id() ?? 1,
                ]);
            }
...
<<<<<<< HEAD
        ...
        try {
            DB::transaction(function () use ($pinjaman) {
                // Kunci baris pinjaman supaya pembayaran angsuran paralel tidak
                // saling menimpa sisa_pinjaman maupun angsuran_ke.
                $pinjaman = KoperasiPinjamanModel::where('id', $pinjaman->id)->lockForUpdate()->first();

                if (! $pinjaman || $pinjaman->status !== 'berjalan') {
                    throw ValidationException::withMessages([
                        'selectedPinjamanId' => 'Pinjaman ini sudah tidak berstatus berjalan.',
                    ]);
                }

                if ($this->jumlah_bayar > (float) $pinjaman->sisa_pinjaman) {
                    throw ValidationException::withMessages([
                        'jumlah_bayar' => 'Jumlah bayar melebihi sisa pinjaman (Rp '
                            .number_format((float) $pinjaman->sisa_pinjaman, 0, ',', '.').').',
                    ]);
                }

                $angsuranKe = $pinjaman->angsuranKeBerikutnya();
                $sisaSetelah = (float) $pinjaman->sisa_pinjaman - (float) $this->jumlah_bayar;
